...
DIALOG_HEIGHT=10
DIALOG_WIDTH=40
PROMPT_MESSAGE="Please enter your disk where to install a bootloader:"
INITIAL_INPUT="/dev/sda" # Optional: default value
...
USER_INPUT=$(dialog --title "Setting up bootloader..." \
                    --clear \
...
case $response in
  0)
    echo "SYS_DISK='$USER_INPUT'" > bootloader.txt_config
    bash list.sh
    ;;
  1)
